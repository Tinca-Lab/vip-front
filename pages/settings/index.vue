<template>
  <div class="px-5 my-28">
    <section class="flex flex-col items-center w-full justify-center">
      ...
      <img
        v-if="user.image !== null" :src="user.image.path" alt="Imagen de perfil"
      ...
      <form class="flex flex-col items-center" @submit.prevent="uploadImage">
        <label class="block mb-2 text-md font-medium text-gray-900 dark:text-gray-300 text-karla" for="files">Cambiar
        ...
        <p id="file_input_help" class="mt-1 text-sm text-gray-500 dark:text-gray-300 text-karla">SVG, PNG, JPG or GIF
          (MAX.
          800x400px).</p>
...
export default {
  ...
  methods: {
    async uploadImage() {
      ...
      await this.$axios.post('api/auth/save-image', formData, {
        headers: {
